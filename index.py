...
bg_music = pg.mixer.music.load('bgcat.mp3')
pg.mixer.music.play(-1)
pg.mixer.music.set_volume(0.1)
collision_sound = pg.mixer.Sound('smw_coin.wav')
...
def growSnake(trail):
    for i in range(len(trail)):
        pg.draw.rect(screen_set, (0, 255, 0), (trail[i][0], trail[i][1], snake_size[0], snake_size[1]))
    for i in range(len(trail)):
...
def changeDirection(key):
    ...
    direction*(key not in (pg.K_UP, pg.K_DOWN, pg.K_LEFT, pg.K_RIGHT) or
    key == pg.K_UP and direction == [0,1] or
    key == pg.K_DOWN and direction == [0,-1] or
    key == pg.K_LEFT and direction == [1,0] or
    key == pg.K_RIGHT and direction == [-1,0])
    )
...
while True:
    ...
    snake_pos = [snake_pos[0] + direction[0]*velocity, snake_pos[1] + direction[1]*velocity]

    trail.append(snake_pos)
    growSnake(trail)

    snake = pg.draw.rect(screen_set, (0, 200, 0),
     (snake_pos[0], snake_pos[1], snake_size[0], snake_size[1]))
    apple = pg.draw.rect(screen_set, (255, 0, 0),
     (apple_pos[0], apple_pos[1], apple_size[0], apple_size[1]))

    if (snake.colliderect(apple)):
        apple_pos = [random.randint(0,640-apple_size[0]), random.randrange(0,480-apple_size[1])]
        score += 1
